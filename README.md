# Kedige Kamogelo — Portfolio

Professional portfolio site for Kedige Kamogelo an aspiring software developer and Computer Science & Informartics Student at the University of Johannesburg.

## About

This repository is a personal portfolio website built with plain HTML, CSS and JavaScript. It showcases projects, skills, education, and contact information for Kedige Kamogelo.
- GitHub: [Kamogelo-Kedige](https://github.com/Kamogelo-Kedige)
- LinkedIn: [Kedige Kamogelo](https://www.linkedin.com/in/kamogelo-kedige-bb2841389/)

## Highlights

- Clean, responsive layout with light/dark theme toggle
- Glassmorphism-inspired card components and hero section
- Projects included in this portfolio:
  - Daily Deli — E‑Commerce Store
  - API‑powered Contact Book
  - Pine City Zoo App
  - My Calculator App
  - Portfolio Generator (no-code idea)

## Tech Stack

- HTML5
- CSS3 (modern features: custom properties, backdrop-filter)
- Vanilla JavaScript (DOM interactions, small progressive enhancements)

## Run Locally

No build step required. To preview locally:

1. Clone the repo:

```powershell
git clone https://github.com/Kamogelo-Kedige/Portfolio-of-Kedige-Kamogelo.git
cd "Portfolio-of-Kedige-Kamogelo"
```

2. Open `index.html` in your browser, or serve it with a static server for best results:

```powershell
# Simple Python static server (python3 required)
python -m http.server 8000
# then open http://localhost:8000
```

## Contributing

This is a personal portfolio — contributions are accepted only by request. If you want to suggest content or fixes, open an issue in the repository or contact me directly.

## License
This project is licensed under the [MIT License](LICENSE).



---

_Built and maintained by Kedige Kamogelo._

